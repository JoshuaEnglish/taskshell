...
list_cmd = quotidia_command.add_parser("list", help="lists current quotidia")
list_cmd.add_argument("columns", nargs=argparse.REMAINDER, help="optional extra colums")
...
directory = quotidiaparser.add_argument(
    ...
    action="store_true",
    default=False,
    help="show directory of the quotidia and quit",
)
...
version = quotidiaparser.add_argument(
    ...
    action="store_true",
    default=False,
    help="show version of the quotidia plugin and quit",
)
...
class QuotidiaCmd(minioncmd.MinionCmd):
    ...
    def __init__(self, completekey="tab", stdin=None, stdout=None, qlib=None):
        super().__init__(
            "quotidia", completekey=completekey, stdin=stdin, stdout=stdout
        )
        self.lib = qlib
    ...
    def do_list(self, text):
        ...
        lister.print_list([getter(q) for q in self.lib.qids.values()], fields)
    ...
    def do_activate(self, text):
        ...
        Activate a given quotidia"""
        self.lib.activate_quotidium(text)

    def do_deactivate(self, text):
        """deactivate QID

        Deactivates a quotidium"""
        self.lib.deactivate_quotidium(text)
...
class Quotidium:
    ...
    def __init__(
        self, qid: str, text: str, days: str, active: bool = True, history=None
    ):
...
class QuotidiaLib(object):
    ...
    def get_quotidia(self):
        for qfile in list(self.directory.glob("*.quotidia")):
            self.qids[qfile.stem] = json.loads(
                qfile.read_text(), object_hook=load_quotidium
            )
    ...
    def add_quotidia(self, qid, text, days):
        ...
        json.dump(q, (self.directory / fname).open(mode="w"), cls=QuotidiaEncoder)
    ...
    def get_todays_quotidia(self):
        ...
        for qid, q in self.qids.items():
            ...
            self.log.debug(
                "%s last run on %s (%s days ago)", qid, q.last_run, days_since_run
            )
            if q.recurrencetype == "DOW":
                self.log.debug("Trying by Day of Week")
                should_have_run = min((daysago(today.weekday(), d) for d in q.days))
